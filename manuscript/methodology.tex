...
where 
\begin{equation}
	\tilde{\boldsymbol{\delta}}_{q} = \tilde{\mathbf{q}} - \mathbf{q}_{a} \: ,
	\label{eq:delta-q-tilde}
\end{equation}
\begin{equation}
	\tilde{\boldsymbol{\delta}}_{d} = \mathbf{d}_{o} - \mathbf{G} \, \mathbf{H} \, \mathbf{q}_{a} \: ,
	\label{eq:delta-d}
\end{equation}
\begin{equation}
...
\begin{equation}
	...
	\label{eq:vector-q-tilde-overdetermined}
\end{equation}
...
\begin{equation}
	\begin{split}
		...
		\tilde{\mathbf{q}} = \left( \mathbf{H}^{\top}\mathbf{W}_{p}^{\top}\mathbf{W}_{p} \, \mathbf{H} \right)^{-1}
		\mathbf{H}^{\top} \mathbf{G}^{\top} \mathbf{u}
	\end{split} \: .
	\label{eq:vector-q-tilde-underdetermined}
\end{equation}
...
Split the observed data $d^{o}_{i}$, $i \in \{1, \dots, D\}$, into $M$ overlapping subsets (or data windows) 
...
Each window has an $D^{m} \times 1$ observed data vector $\mathbf{d}_{o}^{m}$.
...
\begin{equation}
	\mathbf{d}_{p}^{m} = \mathbf{G}^{m} \mathbf{p}^{m} \: ,
...
where $\mathbf{G}'$ is a constant matrix.

PAREI AQUI - PORQUE ALMOÇAR É PRECISO
