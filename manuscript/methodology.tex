...
\begin{equation}
	\mathbf{d}_{w} \approx \mathbf{G}_{L} \, \mathbf{p}_{L} \: ,
	\label{eq:approximated-linear-system-wavelet-domain}
\end{equation}
where 
\begin{equation}
	\mathbf{d}_{w} = \boldsymbol{\mathcal{W}} \, \mathbf{d} \: ,
	\label{eq:vector-dw}
\end{equation}
is the observed data in the wavelet domain; $\boldsymbol{\mathcal{W}}$ is a $D \times D$ orthogonal matrix defining a 
discrete wavelet transform; 
\begin{equation}
	\mathbf{G}_{L} = \mathbf{G}_{s} \, \mathbf{L}^{-1} \: ,
	\label{eq:matrix-GL}
\end{equation}
is defined in terms of a diagonal matrix $\mathbf{L}$ and a sparse matrix $\mathbf{G}_{s}$, which is obtained by setting to zero the 
elements of
\begin{equation}
	\mathbf{G}_{w} = \boldsymbol{\mathcal{W}} \, \mathbf{G} \, \boldsymbol{\mathcal{W}}
	\label{eq:matrix-Gw}
\end{equation}
with absolute value smaller than a given threshold; and
\begin{equation}
	\mathbf{p}_{L} = \mathbf{L} \, \boldsymbol{\mathcal{W}} \, \mathbf{p} \: .
	\label{eq:vector-pL}
\end{equation}

\cite{li-oldenburg_2010} formulate an overdetermined problem (equation \ref{eq:delta-q-tilde-overdetermined}) for obtaining an estimate
$\tilde{\mathbf{p}}_{L}$ for $\mathbf{p}_{L}$ (equations \ref{eq:approximated-linear-system-wavelet-domain} and \ref{eq:vector-pL}) 
by using $\mathbf{H} = \mathbf{I}_{P}$ (equations \ref{eq:reparameterization}),
...
\begin{equation}
	\left( \mathbf{G}_{L}^{\top}\mathbf{G}_{L} \right) \tilde{\mathbf{p}}_{L} 
	= \mathbf{G}_{L}^{\top}\mathbf{d}_{w} \: .
	\label{eq:overdetermined-problem-wavelet-domain}
\end{equation}
They solve this linear system (equation \ref{eq:overdetermined-problem-wavelet-domain}) iteratively by using the conjugate-gradient method
\cite[e.g.,][sec. 11.3]{golub-vanloan2013} and use it to obtain an estimate $\tilde{\mathbf{p}}$ for the parameter vector given by
\begin{equation}
	\tilde{\mathbf{p}} = \boldsymbol{\mathcal{W}}^{\top} \, \mathbf{L}^{-1} \, \tilde{\mathbf{p}}_{L} \: .
