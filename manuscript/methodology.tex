...
\begin{equation}
	\Phi(\mathbf{q}) = \| \mathbf{W}_{d} \left( \mathbf{d} - \mathbf{G} \, \mathbf{H} \, \mathbf{q} \right) \|_{2}^{2} \: ,
	\label{eq:function-Phi}
\end{equation}
and
\begin{equation}
	\Theta(\mathbf{q}) = \| \mathbf{W}_{q} \, \left( \mathbf{q} - \bar{\mathbf{q}} \right) \|_{2}^{2} \: ,
...
$\mathbf{W}_{d}$ is a $D \times D$ matrix defining the relative importance of each observed datum $d_{i}$.
...
\begin{equation}
	\boldsymbol{\nabla} \Gamma(\mathbf{q}) = 
	-2 \, \mathbf{H}^{\top}\mathbf{G}^{\top}\mathbf{W}_{d}^{\top} \mathbf{W}_{d} \left(\mathbf{d} - \mathbf{f} \right) +
	2 \, \mu \, \mathbf{H}^{\top}\mathbf{W}_{p}^{\top}\mathbf{W}_{p}\mathbf{H} \left( \mathbf{q} - \bar{\mathbf{q}} \right) \: .
...
$\left( \mathbf{H}^{\top}\mathbf{G}^{\top}\mathbf{W}_{d}^{\top} \mathbf{W}_{d} \mathbf{G} \, \mathbf{H} \right) \bar{\mathbf{q}}$ ,
we obtain
\begin{equation}
...
\begin{equation}
	\left( \mathbf{H}^{\top} \mathbf{G}^{\top} \mathbf{W}_{d}^{\top} \mathbf{W}_{d} \, \mathbf{G} \, \mathbf{H} + 
	\mu \, \mathbf{H}^{\top}\mathbf{W}_{p}^{\top}\mathbf{W}_{p} \, \mathbf{H} \right) 
...
\begin{equation}
	\begin{split}
		\left[ \mathbf{G} \, \mathbf{H} \left( \mathbf{H}^{\top}\mathbf{W}_{p}^{\top}\mathbf{W}_{p} \, \mathbf{H} \right)^{-1} 
		\mathbf{H}^{\top}\mathbf{G}^{\top} + \mu \left( \mathbf{W}_{d}^{\top} \mathbf{W}_{d} \right)^{-1} \right] \,  
		\mathbf{u} = \tilde{\boldsymbol{\delta}}_{d} \\
		\tilde{\boldsymbol{\delta}}_{q} = \left( \mathbf{H}^{\top}\mathbf{W}_{p}^{\top}\mathbf{W}_{p} \, \mathbf{H} \right)^{-1}
...
Split the observed data $d_{i}$, $i \in \{1, \dots, D\}$, into $M$ overlapping subsets (or data windows) 
formed by $D^{m}$ data each, $m \in \{ 1, \dots, M \}$.
...
Each window has an $D^{m} \times 1$ observed data vector $\mathbf{d}^{m}$.

Let each data window be approximated by a local equivalent layer composed of $P^{m}$ sources, so that
its predicted data vector is given by
\begin{equation}
	\mathbf{f}^{m} = \mathbf{G}^{m} \mathbf{p}^{m} \: ,
	\label{eq:predicted-data-subset-m}
\end{equation}
where $\mathbf{p}^{m}$ is a $P^{m} \times 1$ vector containing the scalar physical properties of the 
equivalent sources in the $m$-th subset and
$\mathbf{G}^{m}$ is an $D^{m} \times P^{m}$ matrix whose elements are computed with equation 
\ref{eq:harmonic-function-g-ij} by using only the coordinates of the observed data and equivalent sources
in the $m$-th subset.
...
$\mathbf{G}^{m} = \mathbf{G}'$ (equation \ref{eq:predicted-data-subset-m}) for all data windows 
(i.e., $\forall \: m \in \{1,\dots,M\}$), where $\mathbf{G}'$ is a $D' \times P'$ constant matrix.
...
\begin{equation}
	t^{m}_{c} = \left( \mathbf{G}' \mathbf{a}' \right)^{\top} \left[ \mathbf{G}' \, \left( \mathbf{G}' \right)^{\top} \right]^{-1} 
...
$\mathbf{W}_{d} = \mathbf{I}_{D'}$, $\bar{p} = \mathbf{0}$ 
(equations \ref{eq:reparameterization-reference}, \ref{eq:delta-q-tilde} and \ref{eq:delta-d}) 
where $\mathbf{I}_{P'}$ and $\mathbf{I}_{D'}$ are identity matrices 
of order $P'$ and $D'$, respectively, and $\mathbf{0}$ is a vector of zeros. 

The method proposed by \cite{leao-silva1989} can be outlined by the following pseudo-code:
\begin{algorithm}
	%\SetAlgoLined
	\Input{}
