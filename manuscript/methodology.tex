...
\citet{takahashi-etal2020, takahashi-etal2022} have deduced the specific relationship between blocks $\mathbf{G}^{\ell}$
and $\mathbf{G}^{-\ell}$ and also between a given block $\mathbf{G}^{\ell}$ and its transposed $\left(\mathbf{G}^{\ell}\right)^{\top}$
...
Matrix $\mathbf{G}_{c}$ (equation \ref{eq:aux-BCCB-system}) is formed by $2D_{B} \times 2D_{B}$ blocks, 
where each block $\mathbf{G}_{c}^{\ell}$, $\ell \in \{(1-D_{B}):(D_{B}-1)\}$ is a $2D_{b} \times 2D_{b}$ circulant matrix.
...
\end{equation}
with blocks $\mathbf{G}_{c}^{\ell}$ having the first column given by
\begin{equation}
	\mathbf{G}_{c}^{\ell}[ \, : \, , \, 1] = \begin{bmatrix}
		\mathbf{G}^{\ell}[ \, : \, , \, 1] \\ 
		0 \\ 
		\left( \mathbf{G}^{\ell}[ 1 \, , \, D_{b}:2] \right)^{\top}
...
\begin{equation}
	\mathbf{G}_{c}^{\ell}[ \, : \, , \, 1] = \begin{bmatrix}
		\left( \mathbf{G}^{\ell}[ \, 1 \, , \, :] \right)^{\top} \\ 
		0 \\ 
		\mathbf{G}^{\ell}[ D_{b}:2 \, , \, 1 ]
	\end{bmatrix}_{2D_{b} \times 2D_{b}} \: , \quad \ell \in \{(1-D_{B}):(D_{B}-1)\} \: .
...
the block columns $\mathbf{G}_{c}[ \, : \, , \, : \, 2D_{b}]$ defined by equations \ref{eq:matrix-G-BCCB-1st-block-column-G}
...
downshifting the first columns $\mathbf{G}_{c}^{\ell}[ \, : \, , \, 1]$ defined by equations 
...
(ii) computing terms $\boldsymbol{\mathcal{F}}_{2D_{B}} \, \boldsymbol{\mathcal{A}} \, \boldsymbol{\mathcal{F}}_{2D_{b}}$
and $\boldsymbol{\mathcal{F}}_{2D_{B}}^{\ast} \, \boldsymbol{\mathcal{A}} \, \boldsymbol{\mathcal{F}}_{2D_{b}}^{\ast}$,
...
Besides, the terms $\boldsymbol{\mathcal{F}}_{2D_{B}} \, \boldsymbol{\mathcal{A}} \, \boldsymbol{\mathcal{F}}_{2D_{b}}$
and $\boldsymbol{\mathcal{F}}_{2D_{B}}^{\ast} \, \boldsymbol{\mathcal{A}} \, \boldsymbol{\mathcal{F}}_{2D_{b}}^{\ast}$
...
PAREI AQUI: É NECESSÁRIO FALAR QUE A PARTIR DE G[:,1] É POSSÍVEL OBTER G$^{T}$[:,1] SABENDO-SE O TIPO DE SIMETRIA DA ESTRUTURA BTTB.
ISSO POSSIBILITA USAR G[:,1] DEFINIR A G$_{c}$[:,1] ASSOCIADA AO PRODUTO Gv=w OU AO PRODUTO GTv=w
...
\begin{algorithm}
	...
	Rearrange the elements of $\mathbf{G}[:,1]$ into the first column $\mathbf{G}_{c}[:,1]$ of the BCCB matrix $\mathbf{G}_{c}$ (equation \ref{eq:aux-BCCB-system})\;
...
%All blocks have null elements, except the first block in the first quadrant, which contains the observed data
%$d_{i}$, $i \in \{1:D\}$.
%The data are arranged along the rows or columns of the first block if the data grid has a $x-$ or $y-$oriented pattern,
%respectively.
%Let us also consider a $2 D_{B} \times 2 D_{b}$ matrix $\boldsymbol{\mathcal{P}}$ having the
%same block structure of $\boldsymbol{\mathcal{D}}$, but containing the elements $p_{j}$ of the parameter vector 
%$\mathbf{p}$, $j \in \{1:P\}$.
%
...
(equation \ref{eq:wiener_takahashi}).%% Methodology
