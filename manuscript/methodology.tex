...
\begin{algorithm}
	...
	$\ell = 1$ \;
	\While{$(a_{m} > \epsilon)$ {\bf and} $(\ell < \mathtt{ITMAX})$}{
		Define the coordinates $(x_{m}, y_{m}, z_{m})$ and index $i_{m}$ of the observation point associated with $a_{m}$ \;
		$\tilde{\mathbf{p}}[i_{m}] \gets \tilde{\mathbf{p}}[i_{m}] + \left( a_{m} \, \boldsymbol{\Delta}\mathbf{z}[i_{m}] \right)$ \;
		$\mathbf{r} \gets \mathbf{r} - \left( \mathbf{G}[:, i_{m}] \, \tilde{\mathbf{p}}[i_{m}] \right)$ \;
		Define the new $a_{m}$ in $\mathbf{r}$ \;
		$\ell \gets \ell + 1$ \;
	}
...
Define a regular grid of $P$ equivalent sources on a horizontal plane $z_{0}$
...
Let $\tilde{\mathbf{p}}$ be the $P \times 1$ parameter vector obtained by solving the underdetermined problem 
(equation \ref{eq:delta-q-tilde-underdetermined}) by using only the data in $\mathbf{d}_{e}$ 
...
\begin{equation}
	\begin{split}
		\left(\mathbf{G}_{e} \, \mathbf{G}_{e}^{\top} + \mu \, \mathbf{I}_{D_{e}} \right) \mathbf{u} = \mathbf{d}_{e} \\
...
\end{equation}
having a maximum absolute value $a_{m} \le \epsilon$, where $\epsilon$ is a predefined tolerance.

PAREI AQUI NESSE PORQUINHO CARAMELADO
...
\begin{algorithm}
	...
	Set a tolerance $\epsilon$ \;
	Define the index $i_{m}$ of the datum having maximum absolute value in $\mathbf{d}$ \;
	Define the list of indices $\mathbf{i}_{r}$ of the remaining data in $\mathbf{d}$ \;
	Define $\mathbf{d}_{e} = \left[ \mathbf{d}[i_{m}] \right] $ \;
	Compute $\tilde{\mathbf{p}}$ (equation \ref{eq:p-tilde-MS94}) \;
	Compute $\mathbf{r} = \mathbf{d} - \mathbf{G}[\mathbf{i}_{r}, :]^{\top} \tilde{\mathbf{p}}$ \;
	Define the maximum absolute value $a_{m}$ in $\mathbf{r}$ \; 
	\While{$(a_{m} > \epsilon)$}{
		Define the index $i_{m}$ of the maximum absolute value $a_{m}$ in $\mathbf{r}$ \; 
		Define the list of indices $\mathbf{i}_{r}$ of the remaining elements in $\mathbf{r}$ \;
		$\mathbf{d}_{e} \gets \begin{bmatrix} \mathbf{d}_{e} \\ \mathbf{d}[i_{m}] \end{bmatrix}$ \;
		Compute $\tilde{\mathbf{p}}$ (equation \ref{eq:p-tilde-MS94}) \;
		Compute $\mathbf{r} = \mathbf{d} - \mathbf{G}[\mathbf{i}_{r}, :]^{\top} \tilde{\mathbf{p}}$ \;
		Define the maximum absolute value $a_{m}$ in $\mathbf{r}$ \; 
	}
