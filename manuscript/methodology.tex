...
\begin{algorithm}
	%\SetAlgoLined
	\Input{}
	Compute $\mathbf{C} = \mathbf{G} \, \mathbf{H}$\;
	Set $\mathbf{r} = \boldsymbol{\delta}_{d}$ and compute $\delta = \nicefrac{\| \mathbf{r} \|}{D}$ \;
	Compute $\boldsymbol{\vartheta} = \mathbf{C}^{\top} \mathbf{r}$ and $\rho_{0} = \boldsymbol{\vartheta}^{\top} \boldsymbol{\vartheta}$ \;
	Set $\tilde{\mathbf{q}} = \mathbf{0}$, $\tau = 0$ and $\boldsymbol{\eta} = \mathbf{0}$ \;
	$m = 1$ \;
	\While{$(\delta > \epsilon)$ {\bf and} $(m < \mathtt{ITMAX})$}{
		Update $\boldsymbol{\eta} \gets \boldsymbol{\vartheta} + \tau \, \boldsymbol{\eta}$ \;
		Compute $\boldsymbol{\nu} = \mathbf{C} \, \boldsymbol{\eta}$\;
		Compute $\upsilon = \nicefrac{\rho_{0}}{\left( \boldsymbol{\nu}^{\top} \boldsymbol{\nu} \right)}$ \;
		Update $\tilde{\mathbf{q}} \gets \tilde{\mathbf{q}} + \upsilon \, \boldsymbol{\eta}$ \;
		Update $\mathbf{r} \gets \mathbf{r} - \upsilon \, \boldsymbol{\nu}$ and $\delta \gets \nicefrac{\| \upsilon \, \boldsymbol{\nu} \|}{D}$ \;
		Compute $\boldsymbol{\vartheta} = \mathbf{C}^{\top} \mathbf{r}$ and $\rho = \boldsymbol{\vartheta}^{\top} \boldsymbol{\vartheta}$ \;
		Compute $\tau = \nicefrac{\rho}{\rho_{0}}$ \;
	...
	\caption{
		...
		for the particular case in which $\mathbf{W}_{d} = \mathbf{I}_{D}$, where $\mathbf{I}_{D}$ is the identity of order $D$, and $\mu = 0$.}
	\label{alg:CGLS}
\end{algorithm}
...
(equation \ref{eq:delta-q-tilde-overdetermined}) is solved by Cholesky factorization (equation \ref{eq:Cholesky-factorization} and \ref{eq:Cholesky-solver})
...
\end{itemize}
Here, we review the computational cost associated with the direct and iterative methods via Cholesky factorization 
and CG (subsection \ref{subsec:linear-system-solvers}), respectively, and use them as references. 
Then we review some strategies for reducing the computational cost of the equivalent-layer technique with respect to
...
Algorithm \ref{alg:SU21}: $\left(\mathbf{G}^{m}\right)^{\top}\mathbf{G}^{m}$ ($2(P')^{2}D'$), 
$\left(\mathbf{G}^{m}\right)^{\top}\mathbf{d}^{m}$ ($2P'D'$),
Cholesky factor $\nicefrac{(P')^3}{3}$ and 
triangular systems $2(P')^{2}$ in line $10$,
saxpy without scalar-vector product in line $11$ ($P'$),
matrix-vector product in line 12 ($2D'P'$):
\begin{equation}
	f_{\mathtt{SU21}} = M \, 
...
Algorithm \ref{alg:C92}: 
find maximum absolute value in line $6$ ($D \log_{2}(D)$) \cite[][p. 420]{press-etal2007},
saxpy in line $11$ ($2D$),
find maximum absolute value in line $12$ ($D \log_{2}(D)$):
\begin{equation}
...
Consider the reparameterization combined with CGLS:
matrix-vector product $\mathbf{G \, H}$ ($2DQP$),
matrix-matrix product $\left(\mathbf{G \, H}\right)^{\top}\left(\mathbf{G \, H}\right)$ ($2Q^{2}D$),
matrix-vector product $\left(\mathbf{G \, H}\right)^{\top}\boldsymbol{\delta}_{d}$ ($2QD$),
matrix-vector product $\mathbf{H} \tilde{\mathbf{q}}$ ($2PQ$)
\begin{equation}
...
\citet{li-oldenburg_2010} solve a normalized inverse problem in the wavelet domain.
...
Algorithm \ref{alg:SOB17}:
entrywise product line $4$ ($D$),
matrix-vector and subtraction line $6$ ($2D^{2} + D$),
entrywise product line $9$ ($D$),
addition line $10$ ($D$),
matrix-vector line $11$ ($2D^{2}$),
subtraction line $12$ ($D$):
\begin{equation}
...
Algorithm \ref{alg:TOB-2020-2022}:
2 FFT (FTT = kappa (4D)log2(4D)) lines 4 and 5,
CONV (= 2FFT + 24D) + inner prod line 8,
3 saxpy (lines 12, 15 and 16),
2 inner prod (lines 14 and 17),
2 CONV lines 13 and 17:
\begin{equation}
