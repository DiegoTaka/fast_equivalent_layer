...
approximate $d_{i}$ by a harmonic function
\begin{equation}
	f_{i} = \sum\limits_{j = 1}^{J} a_{ij} \, p_{j} \: ,
	\quad i \in \{1:I\} \: ,
	\label{eq:predicted-data-fi}
\end{equation}
where,
...
\begin{equation}
	a_{ij} \equiv a(x_{i} - x_{j}, y_{i} - y_{j}, z_{i} - z_{j}) \: ,
	\quad z_{i} < \min\{z_{j}\} \: , \quad \forall i \in \{1:I\} \: ,
	\label{eq:harmonic-function-aij}
\end{equation}
...
\begin{equation}
	...
	\label{eq:predicted-data-vetor}
\end{equation}
where $\mathbf{p}$ is a $J \times 1$ vector with $j$-th element $p_{j}$ representing the scalar physical property
of the $j$-th equivalent source 
and $\mathbf{A}$ is an $I \times J$ sensitivity matrix with element $a_{ij}$ given by equation \ref{eq:harmonic-function-aij}. 

The equivalent-layer technique consists in solving a linear inverse problem to determine a parameter vector $\mathbf{p}$ 
leading to a predicted data vector $\mathbf{f}$ (equation \ref{eq:predicted-data-vetor}) \textit{sufficiently close to} the 
observed data vector $\mathbf{d}$, whose $i$-th element $d_{i}$ is the observed potential field at 
$(x_{i}, y_{i}, z_{i})$.
...
vector $\tilde{\mathbf{p}}$ minimizing a goal function
\begin{equation}
	\Gamma(\mathbf{p}) = \Phi(\mathbf{p}) + \sqrt{\mu} \: \Theta(\mathbf{p}) \: ,
	\label{eq:function-Gamma}
\end{equation}
which is a combination of particular measures of length $\Phi(\mathbf{p})$ and $\Theta(\mathbf{p})$ given by
\begin{equation}
	\Phi(\mathbf{p}) = \| \mathbf{D} \left( \mathbf{d} - \mathbf{f} \right) \|_{2}^{2} \: ,
	\label{eq:function-Phi}
\end{equation}
and
\begin{equation}
	\Theta(\mathbf{p}) = \| \mathbf{P} \left( \mathbf{p} - \mathbf{p}_{r} \right) \|_{2}^{2} \: ,
	\label{eq:function-Theta}
\end{equation}
where $\mu$ is a positive scalar controlling the trade-off between them, 
$\| \cdot \|_{2}$ is the 2-norm (or Euclidean norm), $\mathbf{p}_{r}$ is a $J \times 1$ vector containing reference values
for $\mathbf{p}$, $\mathbf{D}$ is an $I \times I$ matrix defining the relative importance of each observed datum $d_{i}$
and $\mathbf{P}$ is a $J \times J$ matrix introducing some prior information about $\mathbf{p}$, such as those associated
with zeroth-, first- and second-order Tikhonov regularization \cite[e.g.,][p. 104]{aster_etal2019}.
The parameter vector $\tilde{\mathbf{p}}$ minimizing $\Gamma(\mathbf{p})$ (equation \ref{eq:function-Gamma}) can
then be written as follows
\begin{equation}
	\tilde{\mathbf{p}} = \mathbf{p}_{r} + \mathbf{C} \left( \mathbf{d} - \mathbf{A} \, \mathbf{p}_{r} \right) \: ,
	\label{eq:vector-p-tilde-general}
\end{equation}
where \cite[][p. 62]{menke2018}
\begin{equation}
	\mathbf{C} = \left[ \mathbf{A}^{\top} \mathbf{D}^{\top} \mathbf{D} \mathbf{A} + \mu \, \mathbf{P}^{\top}\mathbf{P} \right]^{-1}
	\mathbf{A}^{\top} \mathbf{D}^{\top} \mathbf{D} \: ,
	\label{eq:matrix-C-overdetermined}
\end{equation}
or, equivalently,
\begin{equation}
	\mathbf{C} = \left( \mathbf{P}^{\top}\mathbf{P} \right)^{-1} \mathbf{A}^{\top}
	\left[ \mathbf{A} \left( \mathbf{P}^{\top}\mathbf{P} \right)^{-1} \mathbf{A}^{\top} + \mu \left( \mathbf{D}^{\top} \mathbf{D} \right)^{-1} \right]^{-1} \: .
	\label{eq:matrix-C-underdetermined}
\end{equation}
Evidently, we have considered that all inverses exist in equations \ref{eq:matrix-C-overdetermined} and \ref{eq:matrix-C-underdetermined}.
Matrix $\mathbf{C}$ defined by equation \ref{eq:matrix-C-overdetermined} is commonly used for the cases in which $I > J$, i.e., when
there are more data $d_{i}$ than parameters $p_{j}$ (overdetermined problems).
On the other hand, for the cases in which there are more parameters than data (underdetermined problems), matrix $\mathbf{C}$ is 
usually defined according to equation \ref{eq:matrix-C-underdetermined}.
...
Given an estimate $\tilde{\mathbf{p}}$ (equations \ref{eq:vector-p-tilde-overdetermined} and \ref{eq:vector-p-tilde-underdetermined}),
it is then possible to compute a potential field transformation 
\begin{equation}
	\mathbf{t} = \mathbf{B} \tilde{\mathbf{p}} \: ,
	\label{eq:transformation}
\end{equation}
where $\mathbf{t}$ is a $K \times 1$ vector with $k$-th element $t_{k}$ representing the transformed potential field at
the position $(x_{k}, y_{y}, z_{k})$, $k \in \{1:K\}$, and
\begin{equation}
	b_{kj} \equiv b(x_{k} - x_{j}, y_{k} - y_{j}, z_{k} - z_{j}) \: ,
	\quad z_{k} < \min\{z_{j}\} \: , \quad \forall k \in \{1:K\} \: ,
	\label{eq:harmonic-function-bkj}
\end{equation}
is a harmonic function representing the $kj$-th element of $\mathbf{B}$.

\subsection{Spatial distribution and total number of equivalent sources}

There is no well-established criteria to define the optimum number $J$ or the spatial distribution
...
Generally, the harmonic function $a_{ij}$ (equation \ref{eq:harmonic-function-aij}) is defined in terms of the 
...
Another common approach consists in not defining $a_{ij}$ by using equations \ref{eq:inverse-distance-ij}--\ref{eq:deriv-2-inverse-distance-ij},
...
There are also some less common approaches defining the harmonic function $a_{ij}$ (equation \ref{eq:harmonic-function-aij})
...
A common assumption for most of the equivalent-layer methods is that the harmonic function $a_{ij}$ 
(equation \ref{eq:harmonic-function-aij}) is independent on the actual physical relationship between the
observed potential field and their true sources \cite[e.g.,][]{cordell1992, guspi-novara2009,li_etal_2014}.
Hence, $a_{ij}$ can be defined according to the problem.
...
However, several methods use a function $a_{ij}$ that preserves the physical relationship between the
observed potential field and their true sources.
For the case in which the observed potential field is gravity data, $a_{ij}$ is commonly defined as a component of 
the gravitational field produced at $(x_{i}, y_{i}, z_{i})$ by a point mass or prism located at $(x_{j}, y_{j}, z_{j})$, with unit density.
On the other hand, $a_{ij}$ is commonly defined as a component of the 
...
For all harmonic functions discussed above, the sensitivity matrix $\mathbf{A}$ (equation \ref{eq:predicted-data-vetor}) 
is always dense. For scattered potential-field data, $\mathbf{A}$ does not have a well-defined structure, regardless of
...
show that $\mathbf{A}$ assumes a block-Toeplitz Toeplitz-block (BTTB) structure. In this case, the product of $\mathbf{A}$ and an 
...
Split the observed data $d_{i}$, $i \in \{1, \dots, I\}$, into $M$ subsets with $I^{m}$ data each, with $m \in \{ 1, \dots, M \}$.

The number of data $I^{m}$ forming the subsets are not necessarily equal to each other.

The data forming a given subset are not necessarily close to each other.

For each subset, we have an $I^{m} \times 1$ observed data vector $\mathbf{d}^{m}$.

Let each subset be approximated by an equivalent layer composed of $J^{m}$ equivalent sources, so that
...
The data subsets are defined by square windows of $\sqrt{I'} \times \sqrt{I'}$ adjacent points, all of them having the
...
A different transformation or the same one evaluated at different points require running their method again.
